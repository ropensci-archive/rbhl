...
#' @param ocr (logical) `TRUE` to return the ocr for the item's pages.
#' Setting this to `TRUE` apparently doesn't return any actual ocr text,
#' but leaving parameter here for now.
#' @param parts (logical) `TRUE` to return the item's parts. Setting this
#' to `TRUE` apparently doesn't return any parts text, but leaving
